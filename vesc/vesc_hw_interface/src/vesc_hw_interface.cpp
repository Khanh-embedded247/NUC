...
CallbackReturn VescHwInterface::on_init(const hardware_interface::HardwareInfo& info)
{
  ...
  gear_ratio_ = std::stod(info_.hardware_parameters["gear_ratio"]);//tỉ số truyền
  torque_const_ = std::stod(info_.hardware_parameters["torque_const"]);//hs moment xoắn
  ...
  if (num_rotor_poles_ % 2 != 0)//nếu số cặp cực k chẵn thì error
  {
...
//tạo mảng trống 
  ...
  //kiểm tra số lượng thứ tự các thông số
  ...
  //mảng thông số dự định gồm vị trí,vận tốc, động lực
...
//tạo 1 vecto chứa các giao diện trangj thái vận tóc,lực,vị trí tham chiếu với các khớp nối 
...
//vecto lệnh cho vị trí ,lực,vận tốc tham chiếu vào các khớp
...
hardware_interface::return_type VescHwInterface::read(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/)
{
  ...
  if (joint_type_ == "revolute")//nếu khớp là quay
  {
    position_ = angles::normalize_angle(position_);//đảm bảo vị trí khớp ở góc quay -pi ->pi
  }

  return hardware_interface::return_type::OK;
}
//hàm viết trạng thái của các phần cứng
hardware_interface::return_type VescHwInterface::write(const rclcpp::Time& /*time*/, const rclcpp::Duration& period)
{
  ...
  if (command_mode_ == "position")
  {
    ...
    servo_controller_.setTargetPosition(command_);//đặt vị trí đích
    servo_controller_.control(1.0 / period.seconds());//đk đco theo PID
  }
  ...
  else if (command_mode_ == "effort")
  {
    ...
    // converts the command unit: Nm or N -> A .Chuyển lực thành công 
  ...
  else if (command_mode_ == "effort_duty")//chu kì nhiệm vụ 
  {
