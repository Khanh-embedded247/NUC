...
class TwistToMotors(Node):
    ...
    def __init__(self):
        ...
        self.pub_flmotor = self.create_publisher(Float32, 'flwheel_vtarget', 10)
        self.pub_frmotor = self.create_publisher(Float32, 'frwheel_vtarget', 10)
        self.pub_rlmotor = self.create_publisher(Float32, 'rlwheel_vtarget', 10)
        self.pub_rrmotor = self.create_publisher(Float32, 'rrwheel_vtarget', 10)
    ...
    def spinOnce(self):
        ...
        self.pub_flmotor.publish(Float32(data=self.left))
        self.pub_frmotor.publish(Float32(data=self.right))
        self.pub_rlmotor.publish(Float32(data=self.left))
        self.pub_rrmotor.publish(Float32(data=self.right))
