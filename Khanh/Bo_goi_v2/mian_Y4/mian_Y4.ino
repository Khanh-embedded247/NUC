...
void getRobotState(String currentSiteId) {
  ...
  if (go_go == true) {
    ...
    for (int i = 0; i < siteCount; i++) {
      ...
      http.addHeader("token", "8987ff935fa24f45a4796f9d26a11c11");
      ...
      if (httpCode == HTTP_CODE_OK) {
        ...
        if (!error) {
          ...
          //Nếu giao task thành công và đang trên đường tới Y4
          if (status == 2) {
            Serial.print("before : ");
            if (siteName.equals("ban_1") && finished == 1) {
              pre = true;
              Serial.print("Da giao task thanh cong: ");
            }
          }
          Serial.print("-pre  : ");
          Serial.println(pre);
          //Lúc gọi robot tới Y4 thif keeu
          if (siteName.equals("ban_1") && finished == 2 && pre == true) {
            Serial.println("Robot da den noi");
            keu = true;
            audioPlaying = true;
            go_to_1 = true;
          }
          //Luc dang keu Y4 ma task khac di chuyen thi im
          if (audioPlaying && go_to_1 == true) {
            //Serial.println("-Chuẩn bị im");
            if (resultCode == 9000) {
              if (siteName != "ban_1" && finished != 2) {
                Serial.println("-Im");
                DFPlayer_247.stop();
                keu = false;
                audioPlaying = false;
...
void handleY4State() {
  if (!audioPlaying) {
    DFPlayer_247.playFolder(2, 7);  ////ROBOT YEU CAU LAY DO
    // delay(2000);
    audioPlaying = true;
    Serial.println("ROBOT DA DEN Y4");
    lastSoundTime = millis();
  } else {
    if (millis() - lastSoundTime >= 4000) {
      DFPlayer_247.playFolder(2, 7);  ////ROBOT YEU CAU LAY DO
      Serial.println("ROBOT DA DEN Y4");
      lastSoundTime = millis();
    }
...
void start_up() {
  if (executedAfter10Seconds == false) {
    ...
    if (millis() - programStartTime >= 10000) {
      if (!soundPlayed) {
...
void stopSound() {
  if (buttonPressed) {
    // Kiểm tra trạng thái nút nhấn trong một khoảng thời gian ngắn
    // unsigned long currentTime = millis();
    // if (currentTime - lastButtonCheckTime >= buttonCheckInterval) {
    // lastButtonCheckTime = currentTime;
    // Đảm bảo nút vẫn đang được nhấn
    // nhan = true;
