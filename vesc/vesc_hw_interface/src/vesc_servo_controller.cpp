...
#include "/home/robotic/vesc/vesc_hw_interface/include/vesc_hw_interface/vesc_servo_controller.hpp"
...
//tính toán vị trí hiện tại so với vị trí mục tiêu
  ...
  double u = u_p + u_d + u_i;//setpoint cần tìm
//
  // giới hanj tích phân
  if (antiwindup_)
  {
...
void VescServoController::setGearRatio(const double gear_ratio)//đặt tỷ số truyền
{
  gear_ratio_ = gear_ratio;
  RCLCPP_INFO(rclcpp::get_logger("VescHwInterface"), "[VescServoController]Gear ratio is set to %f", gear_ratio_);
}

void VescServoController::setTorqueConst(const double torque_const)//moments xoắn
{
...
void VescServoController::setRotorPoles(const int rotor_poles)//cặp cực
{
...
void VescServoController::setHallSensors(const int hall_sensors)//số cảm biến
{
...
void VescServoController::setJointType(const int joint_type)//loại khớp
{
  joint_type_ = joint_type;
}

void VescServoController::setScrewLead(const double screw_lead)//vít trợ lực 
{
